...
This database was developed using the following tool:
...
- **Microsoft SQL Server**
  - Download the [Microsoft SQL Server](https://www.microsoft.com/en-us/sql-server/sql-server-downloads).
  - Follow the [installation guide](https://learn.microsoft.com/en-us/sql/database-engine/install-windows/install-sql-server) to install and set it up on your system.
  - Verify your SQL Server installation by launching SQL Server Management Studio (SSMS), connecting to your SQL Server instance, and running a test query to ensure the setup is functioning correctly.
...
3. In SSMS, restore the database:
    ...
    - In the **Source** section, select **Device**, then click the **Browse...** button.
    - Navigate to the `FinalProject` folder within the repository on your local machine.
    - Select the `Airport_Database.bak` file and click **OK**.
    - Follow the prompts to complete the restoration process.
4. Once restored, verify the `Airport` database:
    - Check that it is visible under **Databases** in SSMS.
    - Expand the database to inspect tables, or run a test query to confirm functionality.
